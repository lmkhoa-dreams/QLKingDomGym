...
namespace DAL_QLPhongGym
{
    public class DAL_HoiVien
    {
        ...
        public DataTable GetDanhSachHoiVien()
        {
            ...
            {
                // Dùng LEFT JOIN cho bảng PT vì có người không thuê PT
                string sql = @"
        ...
        public bool ThemHoiVien(string ten, string gioitinh, string sdt, int idGoiTap, int idPT)
        {
            using (SqlConnection conn = new SqlConnection(connStr))
            {
                ...
                try
                {
                    //Thêm vào bảng hội viên và lấy mã hội viên
                    ...
                    cmdHV.Parameters.AddWithValue("@gioitinh", gioitinh);
                    cmdHV.Parameters.AddWithValue("@sdt", sdt);
                    ...
                    cmdDK.Parameters.AddWithValue("@magoi", idGoiTap);
                    cmdDK.Parameters.AddWithValue("@ngay", DateTime.Now.Date);

                    //Không chọn PT thì ko lấy
                    if (idPT == 0)
                        cmdDK.Parameters.AddWithValue("@mapt", DBNull.Value);
                    else
                        cmdDK.Parameters.AddWithValue("@mapt", idPT);
        ...
        public DataTable TimKiemHoiVien(string tenTK)
        {
            using (SqlConnection conn = new SqlConnection(connStr))
            {
                // Chú ý: Nhớ dùng LEFT JOIN giống hệt lúc load danh sách để không bị mất người nha
                string sql = @"
            SELECT hv.mahv as 'Mã HV', hv.ten as 'Họ Tên', hv.gioitinh as 'Giới Tính', hv.sdt as 'SĐT', 
                   gt.tengoi as 'Gói Tập', pt.ten as 'Tên PT', dk.ngay_dangky as 'Ngày ĐK'
            FROM hoivien hv
